...
export function AIChat({ onClose }: { onClose: () => void }) {
  ...
  return (
    <motion.div
      ...
      className="fixed left-0 top-0 bottom-0 w-[400px] bg-[var(--color-punkt-surface)] border-r border-[var(--color-punkt-border)] shadow-2xl z-50 flex flex-col"
    >
      {/* Header */}
      <div className="h-20 border-b border-[var(--color-punkt-border)] flex items-center justify-between px-6 bg-gradient-to-l from-[var(--color-punkt-surface)] to-[var(--color-punkt-bg)]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-display font-bold text-lg leading-none">PUNKT AI</h3>
            <span className="text-xs text-[var(--color-punkt-muted)]">עוזר אישי חכם</span>
          </div>
        </div>
        <button onClick={onClose} className="p-2 text-[var(--color-punkt-muted)] hover:text-white rounded-full hover:bg-white/5 transition-colors">
          <X size={20} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        {messages.map(msg => (
          <div key={msg.id} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
            <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${msg.role === 'user' ? 'bg-[var(--color-punkt-surface-hover)] border border-[var(--color-punkt-border)]' : 'bg-[var(--color-punkt-green)] text-black neon-glow'}`}>
              {msg.role === 'user' ? <User size={16} /> : <Bot size={16} />}
            </div>
            <div className={`max-w-[80%] rounded-2xl p-4 ${msg.role === 'user' ? 'bg-[var(--color-punkt-surface-hover)] border border-[var(--color-punkt-border)] rounded-tr-sm' : 'bg-[var(--color-punkt-green)]/10 border border-[var(--color-punkt-green)]/30 text-[var(--color-punkt-green)] rounded-tl-sm'}`}>
      ...
      {/* Input */}
      <div className="p-6 border-t border-[var(--color-punkt-border)] bg-[var(--color-punkt-bg)]">
